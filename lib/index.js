...
export function apply(ctx) {
  ...
  function usageByModel(sessionId) {
    ...
    let current = null
    // 每个模型 step 会产生两个携带相同 usage 的事件（assistant/chunk 的 usage
    // chunk 与 assistant/message 的 usage），必须按 turn/step 去重，否则每个
    // 请求的 token 会被累计两次，费用估算翻倍。
    const countedSteps = new Set()
    const stepKey = (e) => String(e.data.turn) + '/' + String(e.data.step)
    for (const event of agent.session.events) {
      if (event.type === 'request/context') {
        current = event.data.model
      } else if (event.type === 'assistant/message' && event.data.usage !== undefined) {
        const key = stepKey(event)
        if (!countedSteps.has(key)) {
